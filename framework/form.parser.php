...
require_once(get_template_directory()  . '/framework/form_elements/standard.php');
				...
				if(in_array($field->type, $standard_fields)) {
					...
					if((is_array($file_config) && count($file_config) > 0) || is_object($file_config)) {
						...
						if(!class_exists($file_config->class)) {
							...
							require_once(get_template_directory()  . '/framework/form_elements/'.($field->type).'/'.($file_config->php));
						}
						// create the object
						if(class_exists($file_config->class)) {
