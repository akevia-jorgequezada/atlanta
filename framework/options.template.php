...
function dynamo_template_options_js() {
	...
	if($pagenow == 'admin.php' && isset($_GET['page']) && ($_GET['page'] == 'template_options' || $_GET['page'] == 'dynamo-menu')) {
		...
		wp_register_script('gk-colorpicker', get_template_directory_uri().'/js/back-end/libraries/colorpicker/colorpicker.js', array('jquery'));
		wp_enqueue_script('gk-upload');
		wp_enqueue_script('gk-colorpicker');
		...
		foreach($tabs as $tab) {
			if($tab[2] == 'enabled') {
				...
				if($loaded_data != null && count($loaded_data != 0)) {
					$standard_fields = array('Text', 'Select', 'Switcher', 'Textarea', 'Media', 'WidthHeight', 'TextBlock', 'Color', 'Background', 'Taxonomy', 'HTML', 'Opacity', 'Save');
						...
						foreach($group->fields as $field) {
							if(!in_array($field->type, $standard_fields)) {
								...
								if((is_array($file_config) && count($file_config) > 0) || is_object($file_config)) {
									// load the JS file
									if($file_config->js != '') {
										wp_register_script('gk_'.strtolower($file_config->name).'.js', get_template_directory_uri().'/framework/form_elements/'.($field->type).'/'.($file_config->js));
...
function dynamo_template_options_css() {
	...
	if($pagenow == 'admin.php' && isset($_GET['page']) && ($_GET['page'] == 'template_options' || $_GET['page'] == 'dynamo-menu')) {
		...
		wp_register_style('gk-colorpicker-css', get_template_directory_uri().'/js/back-end/libraries/colorpicker/colorpicker.css');
		wp_enqueue_style('gk-tips-css');
		wp_enqueue_style('gk-template-css');
		wp_enqueue_style('gk-colorpicker-css');
		// register and load external components scripts
		$tabs = $tpl->get_json('options','tabs');
		// iterate through tabs
		foreach($tabs as $tab) {
						...
						foreach($group->fields as $field) {
							if(!in_array($field->type, $standard_fields)) {
								...
								if((is_array($file_config) && count($file_config) > 0) || is_object($file_config)) {
									// load the CSS file
									if($file_config->css != '') {
										wp_register_style('gk_'.strtolower($file_config->name).'.css', get_template_directory_uri().'/framework/form_elements/'.($field->type).'/'.($file_config->css));
