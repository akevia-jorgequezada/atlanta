...
class GKFormInputFonts extends GKFormInput {
	...
	public function output() {
		...
		foreach ($json_data as $font_family) {
			...
			'.($this->required).' 
			'.($this->visibility).'>
				<option value="normal"'.(($type == 'normal') ? ' selected="selected"' : '').'>'.__('Standard fonts', DPTPLNAME).'</option>
				<option value="squirrel"'.(($type == 'squirrel') ? ' selected="selected"' : '').'>'.__('Fonts Squirrel', DPTPLNAME).'</option>
				<option value="google"'.(($type == 'google') ? ' selected="selected"' : '').'>'.__('Google Web Fonts', DPTPLNAME).'</option>
			</select></p>';
			// normal fonts selector
			$output .= '<p><label>'.__('Font family: ', DPTPLNAME).'</label><select id="'.($this->tpl->name).'_fonts_normal_'.($font_family->short_name).'" class="gkInput gkSelect" data-name="fonts_normal_'.($font_family->short_name).'" data-family="'.($font_family->short_name).'" data-type="normal"
			'.($this->required).' 
			'.($this->visibility).'>
				...
				<option value="Helvetica, Arial, sans-serif"'.(($normal == "Helvetica, Arial, sans-serif") ? ' selected="selected"' : '').'>Arial</option>
			...
			$squirrel_fonts = (glob(get_template_directory()  . '/fonts/*' , GLOB_ONLYDIR));
			
			$output .= '<p><label>'.__('Fonts Squirrel: ', DPTPLNAME).'</label><select id="'.($this->tpl->name).'_fonts_squirrel_'.($font_family->short_name).'" class="gkInput gkSelect" data-name="fonts_squirrel_'.($font_family->short_name).'" data-family="'.($font_family->short_name).'" data-type="squirrel"
			'.($this->required).' 
			'.($this->visibility).'>';
				if(count($squirrel_fonts) > 0) { 
					for($i = 0; $i < count($squirrel_fonts); $i++) {
						$short_name = str_replace(get_template_directory()  . '/fonts/', '', $squirrel_fonts[$i]);
