...
function gk_title() {
	...
	if ( is_search() ) {
		...
		$title = sprintf( __( 'Search results for %s', DPTPLNAME ), '"' . get_search_query() . '"' );
		// Add a page number if we're on page 2 or more:
		if ( $paged >= 2 ) {
			$title .= " " . sprintf( __( 'Page %s', DPTPLNAME ), $paged );
		}
		// return the title			
		echo $title;
	}
...
function gk_blog_logo() {
	...
	if(get_option($tpl->name . "_branding_logo_type", 'css') == 'image') {
		...
		if(get_option($tpl->name . "_branding_logo_text_type", 'wp') == 'wp') {
			...
			$logo_text = get_option($tpl->name . "_branding_logo_text_value", '') .'<span>'.get_option($tpl->name . "_branding_logo_text_span_value", '').'</span>'. '<small>' . get_option($tpl->name . "_branding_logo_slogan_value", '') . '</small>';
		}
		// return the logo output
		echo $logo_text;
	}
...
function gk_head_shortcodes() {
	// get access to the template object
	global $tpl;
		wp_register_style('shortcodes-css', get_template_directory_uri().'/css/shortcodes.css');
		wp_enqueue_style('shortcodes-css');
		wp_enqueue_script('shortcodes_js', get_template_directory_uri()."/js/shortcodes.js", array(), false, false);
}
	...
	foreach ( (array) $sidebars_widgets[$index] as $id ) {
		...
		if( !isset($options[$id]) || $options[$id] == '' || $wp_registered_widgets[$id]['gkstate'] == TRUE ) {
			...
			if(isset($tpl->widgets[$index]) && $tpl->widgets[$index] > 1) {
				...
				if ($counter % $tpl->widgets[$index]==$last_amount) $widget_class .= ' latest';
				//
...
function gk_thickbox_load() {
	...
	<script type="text/javascript">
		var thickboxL10n = {
			"next":"<?php _e('Next >', DPTPLNAME); ?>",
			"prev":"<?php _e('< Prev', DPTPLNAME); ?>",
			"image":"<?php _e('Image', DPTPLNAME); ?>",
			"of":"<?php _e('of', DPTPLNAME); ?>",
			"close":"<?php _e('Close', DPTPLNAME); ?>",
			"noiframes":"<?php _e('This feature requires inline frames. You have iframes disabled or your browser does not support them.', DPTPLNAME); ?>",
