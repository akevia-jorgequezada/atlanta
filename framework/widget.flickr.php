...
class DP_Flickr_Widget extends WP_Widget {
	...
	function widget($args, $instance) {
		...
		if ($flickr_id !== '' || $flickr_limit !== '' || $flickr_title !== '' || $flickr_date !== '') {
			...
			$output .= "jQuery('#".$stream_id."').flickrfeed('".$flickr_id."','', { limit:".$flickr_limit.", title:".$flickr_title.", date:".$flickr_date."});"."\n";
			$output .= "});"."\n";
			$output .= "</script>"."\n";
	...
	?>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'flickr_title' ) ); ?>" style="display: block; width: 100%;"><?php _e( 'Show image title:', DPTPLNAME ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'flickr_date' ) ); ?>" style="display: block; width: 100%;"><?php _e( 'Show image date:', DPTPLNAME ); ?></label>
