...
class GavernWP {
	...
	public function __construct() {
		...
		if(
			...
			if(get_locale() != '' && !is_dir(get_template_directory() . '/framework/config/'. get_locale())) {
				array_push($this->problems, 'The framework/config/'.get_locale().' directory doesn\'t exist');
			}
			// 
			if(get_locale() != '' && !is_dir(get_template_directory() . '/framework/options/'. get_locale())) {
				array_push($this->problems, 'The framework/options/'.get_locale().' directory doesn\'t exist');
			}
		}
	 ...
	 * the functions located in framework/functions.php file
	 *
	 * @return null
	 *
	 **/
	 ...
	 * the functions located in framework/filters.php file
	 *
	 * @return null
	 *
	 **/
	 ...
	 private function add_features() {
		...
		add_image_size('portfolio-two', 596, 447, true);
		add_image_size('portfolio-three', 396, 297, true);
		add_image_size('portfolio-four', 296, 222, true);
		add_image_size('portfolio-six', 196, 147, true);
		add_image_size('portfolio-recent', 292, 221, true);
		add_image_size('flex-thumb', 125, 80, true);
	...
	**/ 
	...
	public function get_json($dir, $filename, $lang = true) {		
		...
		$path = get_template_directory() . '/framework/' . $dir . '/' . $lang . $filename . '.json';
