...
//
require_once('framework/classes/class.recaptchalib.php');
...
if(isset($_POST['message-send'])) {
	...
	if(trim($_POST['contact-name']) === '') {
		$validated = false;
		$errors['name'] = __('please enter your name', DPTPLNAME);
	} else {
	...
	if(trim($_POST['email']) === '' || !eregi("^[A-Z0-9._%-]+@[A-Z0-9._%-]+\.[A-Z]{2,4}$", trim($_POST['email']))) {
		$validated = false;
		$errors['email'] = __('please enter correct email address.', DPTPLNAME);
	} else {
	...
	if(trim($_POST['comment-text']) === '') {
		$validated = false;
		$errors['message'] = __('please enter a text of the message.', DPTPLNAME);
	} else {
	...
	if(
		...
		if (!$resp->is_valid) {
			...
			$validated = false;
			$errors['recaptcha'] = __("The reCAPTCHA wasn't entered correctly. Go back and try it again.", DPTPLNAME);
		}
	}
...
<section id="gk-mainbody" class="contactpage">
	...
	<article>
		...
		<p class="gk-thanks"><?php _e('Your message was sent to us successfully.', DPTPLNAME); ?></p>
		<?php else : ?>
		
			<?php if(!$validated) : ?>
			<p class="gk-error"><?php _e('Sorry, an error occured.', DPTPLNAME); ?></p>
			<?php endif; ?>
		
			<form action="<?php the_permalink(); ?>" id="gk-contact" method="post">
				<dl>
					<dt>
						<label for="contact-name"><?php _e('Name:', DPTPLNAME); ?></label>
						<?php if($errors['name'] != '') : ?>
					...
					<dt>
						<label for="email"><?php _e('Email:', DPTPLNAME); ?></label>
						<?php if($errors['email'] != '') : ?>
					...
					<dt>
						<label for="comment-text"><?php _e('Message:', DPTPLNAME); ?></label>
				...
				<p>
					<label>
						...
						<?php _e('Send copy of the message to yourself', DPTPLNAME); ?>
					</label>
				</p>
				...
				<p>
					<input type="submit" value="<?php _e('Send message', DPTPLNAME); ?>" />
				</p>
